...
class WPFB_Item
{
    ...
    public $last_parent = null;
    protected $locked = 0;
    private $_read_permissions = null;

    public static $tpl_uid = 0;
    ...
    public function GetTitle($maxlen = 0)
    {
     ...
     * @return WPFB_Item
     */
    public static function GetByName($name, $parent_id = 0)
    {
        global $wpdb;
     ...
     * @return WPFB_Item
     */
    public static function GetById($id, $cat_or_file)
    {
    ...
    public static function Sort(&$items, $order_sql)
    {
    ...
    public function GetLocalPath($refresh = false)
    {
        return WPFB_Core::UploadDir() . '/' . $this->GetLocalPathRel($refresh);
    }

    public function GetLocalPathRel($refresh = false)
    {
        if ($this->is_file) {
    ...
    protected function TriggerLockedError($throw_on_error=false)
    {
        ...
        trigger_error(
            "Cannot save locked $this to database! ($backtrace)",
            E_USER_WARNING
        );
     ...
     * @return boolean
     */
    public function CurUserCanAccess($for_tpl = false, $user = null)
    {
    ...
    public function CurUserCanEdit($user = null)
    {
        if (is_null($user)) {
    ...
    public function GetUrl($rel = false, $to_file_page = false) // TODO: rawurlencode??
    {
        ...
        if ($rel) {
            ...
            if ($url[0] == '?') {
                ...
                $url = substr($url, 0);
            } // remove trailing slash! TODO?!
        }
        return $url;
    }

    public function GenTpl($parsed_tpl = null, $context = '')
    {
        if ($context != 'ajax') {
    ...
    public function GenTpl2($tpl_tag = null, $load_js = true, $extra_data = null)
    {
    ...
    public function GetThumbPath($refresh = false)
    {
        static $base_dir = '';
     ...
     */
    public function GetChildFiles($recursive = false, $sorting = null, $check_permissions = false, $local_only = false)
    {
        if ($this->is_file) {
        ...
        if($local_only) {
            $where = "(file_remote_uri = '' AND $where)";
        }
     ...
     * @return WPFB_File[]
     */
    public function GetChildFilesFast($recursive = false)
    {
    ...
    public function SetReadPermissions($roles)
    {
        if (!is_array($roles)) {
            $roles = explode('|', $roles);
        }
    ...
    public function CurUserIsOwner($user = null)
    {
        global $current_user;
    ...
    public function ChangeCategoryOrName($new_cat_id, $new_name = null, $add_existing = false, $overwrite = false)
    {
     ...
     */
    public const SCAN_LOCK_TIMEOUT = 600;

    public static $_file_scan_locks;
    public static $_cat_scan_locks;
    public static $_scan_lock_init;

    public static function _removeScanLocks()
    {
        global $wpdb;
    ...
    public function GetScanLockDebug()
    {
        $prefix = $this->is_file ? 'file' : 'cat';
        $slf = "{$prefix}_scan_lock";
        if ($this->is_file) {
